...
Recreates the profile-page "Activity overview" radar: the percentage split of
the last year's contributions across Commits, Code review, Issues, and Pull
requests, fetched from the GitHub GraphQL API. The activity-radar workflow
runs this daily and commits the result so the chart stays live.

Usage:
    GITHUB_TOKEN=<token> python .github/scripts/generate_activity_radar.py

Note: per-type totals only cover contributions the token can see. The default
Actions GITHUB_TOKEN sees public contributions; to include private/org work
(like GitHub's own radar does), set the ACTIVITY_RADAR_TOKEN repo secret to a
classic PAT with repo + read:org scopes.
"""
...
# Matches the tokyonight cards used elsewhere in the README.
BACKGROUND = "#1a1b27"
TEXT = "#c0caf5"
MUTED = "#9aa5ce"
GREEN = "#3fb950"
